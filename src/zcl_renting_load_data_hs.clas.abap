CLASS zcl_renting_load_data_hs DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_renting_load_data_hs IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    DATA: lt_brands    TYPE TABLE OF zrent_brands_hs,
          lt_cars      TYPE TABLE OF zrent_cars_hs,
          lt_customers TYPE TABLE OF zrent_clients_hs.

*   * BRANDS **
    lt_brands = VALUE #(
    ( marca = 'ASTON MARTIN' url = 'https://www.autobild.es/sites/autobild.es/public/st yles/main_element/public/dc/fotos/Aston_Martin-DB9_2013_01.jpg?itok=bGlXq8UF' )
    ( marca = 'AUDI'         url = 'https://acroadtrip.blob.core.windows.net/catalogoimagenes/xl/RT_V_8ca58632abf44d0db6f1b476fd54eec2.jp g' )
    ( marca = 'BENTLEY'      url = 'https://www.autobild.es/sites/autobild.es/public/st yles/main_element/public/dc/fotos/Bentley-Continental_GT-2018-C01.jpg?itok=_kP_edH4' )
    ( marca = 'BUGATTI'      url = 'https://wtfonline.mx/wpcontent/uploads/2018/12/buggatti.jpg' )
    ( marca = 'FERRARI'      url = 'https://api.ferrarinetwork.ferrari.com/v2/networkcontent/medias/resize/5dd552852cdb32285a785d2e-lineup-ferrari-sf90-stradale?apikey=9QscUiwr5n0NhOuQb463QEKghPrVlpaF&wid th=800&height=600' )
    ...
    ( marca = 'HONDA'        url = 'https://cdn.autobild.es/sites/navi.axelspringer.es/ public/styles/480/public/media/image/2017/06/643677honda-civic-type-r-2017.jpg?itok=pmaP2b_K' )
    ...
    DELETE FROM zrent_brands_hs.
    INSERT zrent_brands_hs FROM TABLE @lt_brands.

    SELECT * FROM zrent_brands_hs INTO TABLE @lt_brands.
    out->write( sy-dbcnt ).
    out->write( 'zrent_brands_hs data inserted successfully!' ).
...
*   * CARS **
    ...
    DELETE FROM zrent_cars_hs.
    INSERT zrent_cars_hs FROM TABLE @lt_cars.

    SELECT * FROM zrent_cars_hs INTO TABLE @lt_cars.
    out->write( sy-dbcnt ).
    out->write( 'zrent_cars_hs data inserted successfully!' ).
...
*   * CUSTOMERS **
    ...
    DELETE FROM zrent_clients_hs.
    INSERT zrent_clients_hs FROM TABLE @lt_customers.

    SELECT * FROM zrent_clients_hs INTO TABLE @lt_customers.
    out->write( sy-dbcnt ).
    out->write( 'zrent_clients_hs data inserted successfully!' ).
